...
U_A = eye(m, m);
for i = 1:m-1
    u = [1; A_out(i+1:m, i)]; % Extracting the Householder vector
    tau = t_out(i); % Extracting the corresponding tau value
    
    % Forming the ith column block of U_A
    U_A(i:m, i:m) = U_A(i:m, i:m) - tau * (u * (u' * U_A(i:m, i:m)));
end

% Forming V_A
V_A = eye(m, m);
for i = 1:m-1
    u = [1, A_out(i, i+1:m)]; % Extracting the Householder vector
    tau = r_out(i); % Extracting the corresponding rho value
    
    % Forming the ith row block of V_A
    V_A(i:m, i:m) = V_A(i:m, i:m) - tau * (V_A(i:m, i:m) * (u' * u));
end
